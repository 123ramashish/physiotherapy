...
// We use the compiled JS if possible, but since we are in Next.js, 
// we'll try to just use the schema definition directly if we can, 
// or mock the DB connection.
...
async function seed() {
    try {
        ...
        // Remove existing if they have same slugs
        await Event.deleteMany({ slug: { $in: dummyEvents.map(e => e.slug) } });
        
        const created = await Event.create(dummyEvents);
